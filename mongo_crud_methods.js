const mongo_connector = require('./mongo_config')
const ObjectId = require('mongodb').ObjectId;
const db = {}

db.find_all = (collection_name) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      client = clientObj
      const content = await db.collection(collection_name).find().toArray();
      resolve(content)
...
db.find_by_id = (collection_name, id) => {
  return new Promise(async (resolve, reject) => {
    let client = null;
    try {
      ...
      let { db, client: clientObj } = await mongo_connector.connectToServer();
      client = clientObj
      const content = await db.collection(collection_name).find({"_id" : ObjectId(id)}).toArray();
      resolve(content)
